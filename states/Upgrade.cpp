...
Upgrade::Upgrade(Player *j, std::vector<Arme *> w, sf::Vector2f position) : joueur(j), weapons(w)
{
    ...
    weapons[0]->icoSprite.setPosition(position.x - 320, position.y - 380);
    ...
    weapons[1]->icoSprite.setPosition(position.x - 320, position.y - 140);
    ...
    weapons[2]->icoSprite.setPosition(position.x - 320, position.y + 100);
}
...
void Upgrade::moveUp()
{
    ...
    choixSelected--;
    if(choixSelected < 0) choixSelected = 2;
    options[choixSelected][0].setFillColor(sf::Color(140, 70, 20));
}
...
void Upgrade::moveDown()
{
    options[choixSelected][0].setFillColor(sf::Color::Black);
    choixSelected++;
    if(choixSelected >= 3) choixSelected = 0;
    options[choixSelected][0].setFillColor(sf::Color(140, 70, 20));
}
...
void Upgrade::draw(sf::RenderWindow &window)
{
    for (int i = 0; i < 3; i++)
    {
        ...
        window.draw(weapons[i]->icoSprite);
    }
}