...
{   bool gauche = sf::Keyboard::isKeyPressed(sf::Keyboard::Left); //Récuperation des fleches pressees
    ...
    if (gauche && droite){ //Si deux flèches opposees activees alors elles s'annulent
        gauche = 0;
        droite = 0;
    }
    if (haut && bas){
    ...
    if (droite){ //Quand fleche de droite appuyee
        if (haut){ //Et que fleche haut appuyee, on monte en diagonale
        ...
        else if (bas){ //Et que fleche bas activee,on descend en diagonale
            location.x += coef * speed;
    ...
    if (bas && (!droite) && (!gauche)){ //Si juste fleche bas activee
        location.y += speed;
    }

    if (location.x >= 3571.){ //Les 4 if c'est pour gerer les bordures de la map, pour pas que le perso sorte
        location.x = 3571.;  
    }   
