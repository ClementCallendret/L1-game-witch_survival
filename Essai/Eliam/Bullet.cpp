...
Bullet::Bullet(float X, float Y, float R, float D, float S, int Life, Ennemi &Cible, Animation &a, ModeDeTir mDT, Player* j) : x(X), y(Y), rayon(R), degats(D), speed(S), life(Life), cible(&Cible), bul_anim(a), mdt(mDT), tireur(j)
{
